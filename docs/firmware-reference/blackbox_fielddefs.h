/*
 * Snapshot of betaflight/src/main/blackbox/blackbox_fielddefs.h
 *
 * Source: Bskimp/betaflight, branch master
 * Commit: e92c108873a876922ff8e07ef19b98a02d5a4ddb
 * File SHA: c6f977a9112c0b4ead3a54d494995843133e6d16
 * Captured: 2026-05-15
 *
 * This is a verbatim copy held as a reverse-lookup reference for:
 *  - Field name conventions (FLIGHT_LOG_FIELD_CONDITION_* enums)
 *  - Predictor / encoder enums (SIGNED_VB, PREDICT(PREVIOUS), etc.)
 *  - Event-frame data unions (used to derive M1.4's event/annotation track)
 *
 * Refresh on each major BF release; bump the commit / capture date above when
 * you do. Drift between this snapshot and live firmware = potential parser
 * regression — when validate-parser flags a mismatch in CI, the divergence is
 * usually here first.
 *
 * Upstream license: GPL-3.0-or-later (Cleanflight/Betaflight). The original
 * header comment is preserved below.
 */
