...
const data = computed<AlignedData>(() => {
  ...
  for (const t of allTraces.value) {
    series.push(smoothTrace(resampleOntoRef(t.entry.log, refTime.value, t.spaMultiplier), smooth));
    series.push(smoothTrace(resampleOntoRef(t.entry.log, refTime.value, t.iTermArr), smooth));
  }
